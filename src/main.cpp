...
#if defined(NDEBUG)
# error "Prosperity cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Prosperity Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Prosperity: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Prosperity: find block wanted by given orphan block
...
{
    int64_t nSubsidy = 0.6736 * COIN;

    //nSubsidy = (nCoinAge * (0.01 * COIN)) / 365;

    if(pindexPrev->nHeight > 1051200)
        nSubsidy = 4.5881 * COIN;
...
// Prosperity: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal Prosperity miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Prosperity: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Prosperity: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // Prosperity: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // Prosperity: track money supply and mint amount info
...
// Prosperity: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // Prosperity: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // Prosperity: compute stake entropy bit for stake modifier
    ...
    // Prosperity: compute stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Prosperity: check proof-of-stake
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // Prosperity: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// Prosperity: attempt to generate suitable proof-of-stake
...
{
    RenameThread("Prosperity-loadblk");
